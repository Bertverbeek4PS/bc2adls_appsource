codeunit 85571 "AZD Session Manager Tests"
{
    Subtype = Test;
    ...
    begin
        ...
        ADLSERun.SetRange("Table ID", Database::"Reason Code");
        ADLSERun.FindLast();
        LibraryAssert.AreEqual("AZD Run State"::Failed, ADLSERun.State, 'Last run should be failed');
    end;
    ...
    begin
        LibraryTestInitialize.OnTestInitialize(Codeunit::"AZD Session Manager Tests");

        if IsInitialized then
            exit;

        LibraryTestInitialize.OnBeforeTestSuiteInitialize(Codeunit::"AZD Session Manager Tests");

        IsInitialized := true;
        Commit();

        LibraryTestInitialize.OnAfterTestSuiteInitialize(Codeunit::"AZD Session Manager Tests");
    end;
}
